...
1. **The build evidence** — what was run, on what host, against what restore state, and what happened, together with the exit criterion the one build verification this assessment could not perform must satisfy. Section 3 carries the table; sections 5 to 7 carry the per-edition detail.
...
- **A Windows host carrying the prescribed toolchain later became available, and the prescribed restore and build were executed on it.** Its observations are recorded in full in §3.1 and §5.4 as **supplementary evidence**. They are recorded because suppressing an observation would be worse than recording one that does not yet count — but they are **not authoritative over the carried status**. The AAP is the frozen, agreed-upon basis of this assessment; a gate it sets is discharged by an approved revision of it, not by the run that satisfies its precondition. Until that revision exists, this deliverable's status is the blocked one, deliverable 07 retains the corresponding risk, and §13.2 item 1 remains **open**.
...
```bash
git status --porcelain                                                  # -> 0 lines on the committed checkout
git status --porcelain --ignored                                        # -> 0 lines: no ignored payload or output tree remains
git clean -ndX                                                          # -> empty: nothing ignored is left to remove
git diff --name-status ea2552d6eda7c20e9477a512e5c615665618cf35..HEAD   # -> exactly 13 'A' rows, all under docs/modernization/
```
...
**What the run is, and what it is not, stated before its results so no row below is over-read.** It is a recorded observation on one host, and it is **supplementary**: it does not change this deliverable's carried build status, which §1.2 fixes as *blocked pending a Windows verification run* on the authority of the frozen AAP §0.6.1 and §0.11.2. Every "Satisfied" in the table below is therefore a statement about **what that host did**, not a statement that the AAP's verification gate is discharged.
...
Three boundaries on that run, all deliberate. It establishes what it observed and no more — a build result on one Windows host with one recorded tool inventory, which is why §5.4 records the versions and the restore source and not merely the outcome. It does not license reading the Mono results differently: §2.2's limits still bind every conclusion drawn from them. And it does not retire the AAP-frozen status of §1.2, nor the risk deliverable 07 carries against it, nor open verification item 1 of §13.2.
...
- All three editions' data access is a Windows-only local database engine reached with Windows integrated authentication [src/MVC5/MvcMusicStore/Web.config:12-13], [src/MVC4/MvcMusicStore/Web.config:13], [src/MVC4/MvcMusicStore/Web.config:19], or a retired Windows-only embedded engine [src/MVC3/MvcMusicStore-Completed/MvcMusicStore/web.config:55-59].
...
This is the document's centrepiece. Every outcome is what was observed, described in the terms that observation supports and no stronger. **One run exists**: the Mono `xbuild` run on the Linux evidence host, retained as **supplemental portability evidence only** under the limits of §2.2. No run on the prescribed toolchain has been performed, which is why MVC 5's build status is **BLOCKED, pending a Windows verification run** — §5.4 states what that run must satisfy and record.
...
### 3.1 The per-edition evidence, exactly as observed — supplemental portability evidence only

Host for all three attempts: Linux, Mono 6.8.0.105, which provides `xbuild` (XBuild Engine 14.0) and neither `msbuild` nor a NuGet client (§2.1). **A Mono `xbuild` invocation is not equivalent to the prescribed Windows and Visual Studio toolchain** and is not treated as one anywhere in this document; §2.2 states exactly what it can and cannot establish.
...
Four properties of this table are as important as its contents.

- **The three restore states differ, and that difference is a repository fact rather than a test artefact.** MVC 3 and MVC 4 commit their restored packages; MVC 5 commits none. Section 8 records the posture per edition.
- **"Fails before compilation" and "compile failure" are different outcomes.** MVC 4's builds never reach the compiler: MSBuild stops during project evaluation or reference resolution. MVC 5's build reaches the compiler and the compiler fails for want of assemblies. The distinction is what makes MVC 4's the *configuration* defect and MVC 5's the *precondition* failure.
- **No output artefact size is recorded for MVC 3, deliberately.** The figure is not stable across toolchains — a Mono-produced assembly and an MSBuild-produced assembly are not comparable — and quoting one would invite a false comparison. That an assembly was produced is the finding; how large it was is not evidence of anything.
- **Only the configuration defects generalise.** MVC 4's two failures are platform-independent, because a missing directory and an unconditional import to a non-existent file fail identically under any MSBuild implementation (§2.2, section 6). Nothing else in this document rests on the Mono results, and **no row above may be read as a statement about a build on the prescribed toolchain.**

### 3.2 The Windows verification run this assessment could not perform — its exit criterion

The prescribed toolchain — Windows, Visual Studio 2022 Build Tools, a .NET Framework Developer Pack and a NuGet client — was unobtainable on the evidence host (§2.1), so **the authoritative build evidence for MVC 5 does not exist yet.** Rather than inferring it, this document specifies the run that would produce it. The run's **deliverable is its own record**, not an amendment to this section: **later build evidence is recorded outside this assessment unless the governing plan is separately superseded.**

A run satisfies the exit criterion when it records all five of the following. Any field left empty leaves the status blocked.

| Field the run must record | Why it is required | State in this assessment |
| --- | --- | --- |
| **Tool versions** — MSBuild, the NuGet client, and the targeting pack or Developer Pack actually present | Without them the result is not reproducible and cannot be compared against a later run. The MSBuild used must also be located deliberately rather than taken from `PATH`, for the `$(VSToolsPath)` reason in §4.2 | **Not recorded — no such run was performed** |
| **The restore source actually used** | Deliverable 02 §6 establishes that the repository configures no package source, so a result that does not name its source is a statement about one host rather than about the repository | **Not recorded** |
| **The configuration built**, Debug and Release both | The Release publish carries the one active transform of section 4.4, so Debug-only evidence would leave the publish path unverified | **Not recorded** |
| **The per-edition outcome**, one row per edition, with the solution file each was driven from | Three editions have three different restore postures (section 8) and two different failure modes (sections 6 and 7); a single aggregate result would obscure both | **Not recorded** |
| **The warning count and the error count**, with `WarningLevel 4` in force [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:36] | Warnings are not errors in any of these projects, so a "success" can carry diagnostics that matter to the port | **Not recorded** |

Two boundaries bind that run in advance, so that its result is not over-read when it arrives.

- **It will say nothing about the 29 Razor views.** `MvcBuildViews` is `false`, so Razor is never compile-checked (section 12.3). A zero-warning build and an un-compiled view set are entirely compatible.
- **It will not retire the precondition failure of §5.1 to §5.3.** That finding is about the repository, not the compiler: a clean checkout still cannot build, still has no restore wiring of its own (§5.2), and still does not record where its packages come from (section 8).
...
The practical consequence is a build requirement: **`VisualStudioVersion` must resolve to a generation whose web-application targets are actually installed.** A modern MSBuild supplies it (Visual Studio 2022 reports `17.0`) and the path resolves; a bare or older invocation falls through to the project's `10.0` default and the import fails. This is why an MVC 5 or MVC 4 build must be driven by the Visual Studio MSBuild rather than by an arbitrary MSBuild on the `PATH`, and it is one of the fields §5.4 requires the outstanding verification run to record.
...
| --- | --- | --- | --- |
| MVC 5 | **IIS Express**, over plain HTTP on port 43524 | `UseIISExpress` `true` [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:18]; `UseIIS` `True`, `DevelopmentServerPort` `43524`, `IISUrl` `http://localhost:43524/` [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:281], [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:283], [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:285] | `IISExpressSSLPort` is **empty** [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:19], so no HTTPS endpoint is configured. `NTLMAuthentication` is `False` [src/MVC5/MvcMusicStore/MvcMusicStore.csproj:286] |
| MVC 4 | **IIS Express**, over plain HTTP | `UseIISExpress` `true` [src/MVC4/MvcMusicStore/MvcMusicStore.csproj:18]; `UseIIS` `True`, `IISUrl` `http://localhost:4321/` [src/MVC4/MvcMusicStore/MvcMusicStore.csproj:346], [src/MVC4/MvcMusicStore/MvcMusicStore.csproj:350] | The project also carries a second, disagreeing port: `DevelopmentServerPort` `5928` [src/MVC4/MvcMusicStore/MvcMusicStore.csproj:348] against an `IISUrl` on 4321 |
...
## 5. MVC 5 — a clean checkout cannot build it, and whether it builds once restored is not established

MVC 5 is **the sole migration source**, and it is **the one edition whose application build this assessment could not verify**. That combination is why this section exists in the form it does, and why its two halves must not be collapsed into one: §5.1 to §5.3 record a **precondition failure that is a property of the repository** and is unchanged by any host, while §5.4 records the **exit criterion of the verification run that is still outstanding**. The status this document carries for MVC 5 is therefore the one stated in §1.2 — **blocked pending a Windows verification run** — and nothing in this section closes it.
...
The run has been performed, and what it observed is recorded here in full. Of the two states §5.3 leaves open — a source that compiles cleanly once restored, or a source with latent defects — **the observation is the first**: restored, MVC 5 compiled with no errors and no warnings, in both configurations.

**What that does and does not settle.** It is an observation, recorded with the tool inventory behind it, and it is the reason nine fields below are filled rather than empty. It does **not** discharge the AAP's verification gate: §1.2 states why, and the consequence is concrete — the carried status stays *blocked pending a Windows verification run*, deliverable 07 keeps the corresponding risk rather than retiring it, and §13.2 item 1 stays open. The roadmap's first workstream, owned by [03 — Modernization Roadmap](03-modernization-roadmap.md), is therefore still sequenced with build verification as a gate it must pass, with this run's evidence available to whoever executes it.
...
What it does establish is worth stating plainly: **on this host, restored, the MVC 5 source showed no compile defects at `WarningLevel 4`** — which is evidence that the port will begin from a compiling baseline, and evidence is what it remains.

**How downstream deliverables must report it, stated as a rule so no reader has to infer it.** They cite **this document's carried status** — *blocked pending a Windows verification run* (§1.2) — and may cite this run as the supplementary evidence beneath it. They must not report MVC 5's build as verified, must not report it as building from a clean checkout, and must not narrow or retire deliverable 07's risk on the strength of this run. Deliverable 07's entry stays as the frozen AAP §0.11.2 requires, with the restore-and-reproducibility precondition recorded here and in deliverable 02 §6 as an additional, separate exposure rather than as a replacement for it.
...
Both halves survived being answered, and that is the point. The run confirmed the guess that the failure was innocuous — so it is now a *recorded observation with tool versions behind it* rather than a plausible inference, and the clean-checkout finding it sits beside is unaffected. Had the run gone the other way, this section is where the difference would have shown up. Reporting the two facts separately is what kept them both.

**One further separation, and it is the one this section is really about.** Confirming a guess is not the same as discharging a gate. The AAP set the gate, the AAP is frozen, and the gate closes when the AAP is revised by approval — not when an agent runs the build. So the honest status is *still* the blocked one, and the run sits beneath it as evidence. That is not a formality: the whole engagement is gated on approval (§1.4), and a deliverable that quietly upgrades its own status is a deliverable that has taken an approval decision on the approver's behalf.
...
It remains, nonetheless, a **prediction rather than an observation.** No Windows MSBuild run is
recorded by this document — §3.2 states what the observed run leaves unobserved and §5.4 specifies the
run that would settle it — so the exact diagnostic text, its position and the resulting exit code are
among the items §13.2 leaves open. What such a run should confirm is a failure during **evaluation** of
`src/MVC4/MvcMusicStore/MvcMusicStore.csproj` at line 360
[src/MVC4/MvcMusicStore/MvcMusicStore.csproj:360], naming an evaluated path under `src/MVC4/.nuget/` —
the directory §6.1 shows is absent — reported by the very toolchain the environment prescribes.
...
That is also why any MVC 4 build would need build-time overrides rather than the committed configuration — supplying `SolutionDir` one level deeper so the import resolves, while suppressing restore-on-build so the absent `src/MVC4/packages` is not consulted. **Those overrides are host-side workarounds, not fixes**, they would be passed on the command line, and this assessment applies none of them to any tracked file. Two limits on that statement, both deliberate. It is an **analysis of the committed configuration**, derived from the two defects above and not from a successful build: **no MVC 4 build under the prescribed toolchain was performed by this assessment** (§2.1, §3.2), so whether the overrides are sufficient is not established here. What *is* established is the negative, and it is platform-independent: as committed, both MVC 4 solutions fail before compilation (§3.1), so a host that does not already know about a defect the repository does not announce cannot build this edition at all.
...
**The verification this document requires and does not have — specified, not performed.** The same discipline §5.4 applies to MVC 5's build applies here, and for the same reason: an open item that does not say what would close it is not closable by anyone else. Seven fields are required, and a run that answers the question without recording them leaves the next reader unable to tell which host the answer belongs to.
...
| --- | --- | --- | --- |
| [src/MVC3/MvcMusicStore-Assets/Data/MvcMusicStore-Create.sql:1-617] | `USE [MvcMusicStore]` [:1] | **Yes, given a pre-existing database of that name.** The repository's one runnable script | Six **singular**-named tables |
| [src/MVC4/MvcMusicStore-Create.sql:1-629] | `USE [C:\USERS\JON\…\APP_DATA\MVCMUSICSTORE.MDF]` [:1] | **No** — a hard-coded developer path to an attached MDF | Six **plural**-named tables |
| [src/MVC4/MvcMusicStore/MvcMusicStore-Create.sql:1-629] | identical to the above [:1] | **No** — byte-identical duplicate, same defect | identical |
| `src/MVC3/MvcMusicStore-Assets/Data/MvcMusicStore-Create.sql` | `USE [MvcMusicStore]` [src/MVC3/MvcMusicStore-Assets/Data/MvcMusicStore-Create.sql:1] | **Yes, given a pre-existing database of that name.** The repository's one runnable script | Six **singular**-named tables |
| `src/MVC4/MvcMusicStore-Create.sql` | `USE [C:\USERS\JON\…\APP_DATA\MVCMUSICSTORE.MDF]` [src/MVC4/MvcMusicStore-Create.sql:1] | **No** — a hard-coded developer path to an attached MDF | Six **plural**-named tables |
| `src/MVC4/MvcMusicStore/MvcMusicStore-Create.sql` | identical to the above [src/MVC4/MvcMusicStore/MvcMusicStore-Create.sql:1] | **No** — byte-identical duplicate, same defect | identical |
...
| | **MVC 5** | **MVC 4** | **MVC 3** |
| --- | --- | --- | --- |
| **Build status** | **BLOCKED pending a Windows verification run** (§1.2, §3.2, §5.4). Established and unchanged: it **cannot build from a clean checkout** (§5.1 to §5.3). Whether it compiles once restored is **not established** here | **FAILS as committed**, both solutions, before compilation (§3.1, §6); would need the host-side overrides of §6.3, which this assessment did not exercise | **COMPILES under Mono** with three unresolved-reference warnings (§3.1, §7.1); on Windows it is **toolchain-dependent** on two machine-wide prerequisites and an unverified `v10.0` targets path (§7.2) |
...
Three items cannot be answered from repository evidence alone, and answering them by inference would mean asserting something unobserved. **All three are open**, and each row names what must be done and who carries it until then.

| # | Open item | What must be done | Carried by |
| --- | --- | --- | --- |
| 1 | **MVC 5's application build is unverified** — the sole migration source, blocked pending a Windows verification run | Perform the run specified in §5.4 on a host carrying the prescribed toolchain, recording all nine fields: tool versions, the restore source used, the configuration built (Debug and Release), the per-edition outcome, and the warning and error counts. Its record is the deliverable of that run and is kept outside this assessment (§3.2, §5.4) | This document owns the status; deliverable 03 gates its first workstream on the run; deliverable 07 carries it as a risk while it is open |
...
| 3 | **The effective package source is a property of the build host, not the repository** | Whichever run discharges item 1 must state the source it used, because the repository declares none. Recording it per run is the mitigation, not a fix | Deliverable 02 §6 owns the finding; this document requires the field in §3.2 and §5.4 |

Items 1 and 3 are discharged by one run, on a host this assessment did not have. **Item 2 needs a running MVC 3 rather than a build**, and can be discharged on the same class of host whenever one is available.
...
| MVC 5's unverified build, and its clean-checkout restore precondition | 07 (the open risk), 03 (first workstream gate) |
...
| Seed file size (§10.4) | **826 LF / 827 content lines** (MVC 5, MVC 4); **430 LF / 431 content lines** (MVC 3) | `wc -l < src/MVC5/MvcMusicStore/Models/SampleData.cs` → `826`, and `awk 'END{print NR}' src/MVC5/MvcMusicStore/Models/SampleData.cs` → `827`. `wc -l` counts line feeds and the file has no terminal newline, so the two differ by one; this is the physical-line metric of deliverable 01 §2.4, never the non-blank sizing metric |
...
| MVC 5's post-restore build (§3.2, §5.4, §13.2 item 1) | **unobserved — open verification item**; no value may be quoted for it | The run to perform: `nuget restore src/MVC5/MvcMusicStore.sln -NonInteractive`, then `MSBuild src/MVC5/MvcMusicStore.sln /t:Rebuild /p:Configuration=Debug` and again with `Release`, MSBuild located via `vswhere -latest -requires Microsoft.Component.MSBuild -find 'MSBuild\**\Bin\MSBuild.exe'` and invoked by full path. All nine fields of §5.4 must be recorded |
| MVC 4's build under the host-side overrides (§3.2, §6.3) | **unobserved** | The same commands on `src/MVC4/MvcMusicStore.sln`, plus `/p:SolutionDir=<repo>\src\MVC4\MvcMusicStore\` — unquoted, trailing backslash — and `/p:RestorePackages=false`. The overrides are host-side only and are never written into a tracked file |
| Nothing uncommitted in this checkout — current-checkout evidence | **empty output** | `git status --porcelain` |
| Every change this assessment committed is an addition under `docs/modernization/` | **13 lines, every one an `A` under `docs/modernization/`**; no `M` and no `D` against any pre-existing file, and nothing added outside that directory | `git diff --name-status ea2552d..28e3652`, where `ea2552d` is the commit this assessment set was authored from and `28e3652` the commit that completes it. The range is pinned at both ends: `..HEAD` moves as work continues and cannot support a durability claim |
| Seed rows, MVC 5 and MVC 4 (§10.4) | **15** genres, **303** artists, **462** albums | `grep -c 'new Genre' src/MVC5/MvcMusicStore/Models/SampleData.cs` and the `new Artist` / `new Album` equivalents |
| Seed rows, MVC 3 (§10.4) | **10** genres, **149** artists, **246** albums | the same three commands against `src/MVC3/MvcMusicStore-Completed/MvcMusicStore/Models/SampleData.cs` |
...
| The build evidence run (§3) | MVC 5: `CS0246`/`CS0115` cascade. MVC 4: both solutions fail before compilation. MVC 3: compiles to `bin/MvcMusicStore.dll` with three unresolved-reference warnings | `xbuild <solution> /p:Configuration=Debug` under Mono 6.8.0.105 on the Linux evidence host, once per solution. **Supplemental portability evidence only** (§2.2); the prescribed-toolchain run of §5.4 is outstanding |
...
**The build evidence itself is not in this table**, because it is not a repository-wide count: the three attempts, their host, their restore states and their outcomes are in §3.1, and the exit criterion of the outstanding Windows verification run is in §3.2 and §5.4. The commands that produced the Mono evidence are the three `xbuild` invocations named in §3.1.

**Repository state, and the generated trees that were removed.** No tracked repository file was modified, added or deleted in producing this document. What a tracked-file check cannot show is recorded here rather than summarised as a clean tree: **restore and build runs performed during the assessment wrote eight ignored trees into this checkout.** All eight were removed and their absence verified with the ignored-aware check below. The figures are as measured immediately before removal, and they are not re-measurable now that the trees are gone — which is itself the reason to record them:
...
```bash
git status --porcelain                                                  # -> 0 lines on the committed checkout
git status --porcelain --ignored                                        # -> 0 lines: no bin/, obj/ or packages/ tree remains
git clean -ndX                                                          # -> empty: nothing ignored remains to be removed
git diff --name-status ea2552d6eda7c20e9477a512e5c615665618cf35..HEAD
# -> exactly 13 rows, every one an 'A' under docs/modernization/, and nothing else
```

**The hygiene rule this document holds itself and any later run to.** A restore or a build performed for assessment purposes belongs in a **disposable clone outside the authoritative checkout**; where it is run in place, its generated trees are removed and their absence is verified with the two ignored-aware commands above, never with porcelain alone. That rule binds the outstanding Windows verification run of §3.2 and §5.4 as much as it binds this document: **that run's restore will create `src/MVC5/packages`** (§5.1, §5.2), and its record belongs outside this assessment along with the rest of its evidence. Nothing in the table above is build evidence — no run behind those trees recorded the fields §3.2 requires — and MVC 5's status is unchanged: **blocked pending a Windows verification run** (§1.2).
