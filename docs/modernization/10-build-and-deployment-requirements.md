...
The user directed that no code changes be made before the assessment is approved, and the project's environment setup instructions restate the same gate independently. **No tracked repository file was modified, added or deleted in producing this document.** What the assessment nonetheless *wrote* into the checkout is generated content, and it is reported rather than folded into a clean-tree claim: **restore and build runs were performed against this checkout, and they left eight ignored trees behind** — a `bin` and an `obj` under each of the three editions' projects, plus a restored `packages` tree under `src/MVC4` and another under `src/MVC5`. **No extent is published for the six `bin` and `obj` trees**, and that is a deliberate withdrawal rather than an omission: they are gone, nothing immutable was retained of them, and a `bin`/`obj` extent is a property of the host that supplied the referenced assemblies and of the path the checkout sat at rather than of this repository. Their removal is deliberately **not** recorded here as a discharged, one-time act: §3.2's post-freeze run was performed in place as well and writes the same set again, so removing them and verifying their absence with the ignored-aware check below is a **standing requirement on whoever commits this assessment** rather than an observation this document can make on its own behalf. Appendix A carries the per-tree record, the ignore rule per tree, the commands, the extent that *is* reproducible — the two restored `packages` trees, which a fresh restore re-derives — and the two reasons no build-output extent is published.
...
**Restore emitted advisory warnings and still exited `0`, which is why the warnings are recorded as a property of the run rather than folded into its outcome.** They are output of this run and not an advisory assessment: the pins, their version-risk posture, and the reason no advisory identifier is asserted for the pins this repository ships, belong to [02 — Dependency Inventory](02-dependency-inventory.md) §8.2, and nothing here revises it. This document quotes none of the identifiers, severities or counts those warnings carried, and **that abstention is scoped to the legacy pins**: two deliverables do assert advisory identifiers, each against a target-state subject this repository does not ship, and between them they are this corpus's only owners of them — [04 — .NET 8 migration strategy](04-dotnet8-migration-strategy.md) §7.4 carries the set against a **target-state transitive package pin** reached only on the conditional incremental path, a version the plan selects rather than one the repository already ships, verified at an exact version against primary sources and named so that its remediation floor can be held as a precondition on that path; and [06 — Azure hosting recommendations](06-azure-hosting-recommendations.md) §12.3, with its §6.10.1, carries the identifiers a **binary** scan measured in the two privileged release-path utilities that estate would install, named so that each utility's admission is gated on the finding being closed. Neither subject is a pin this repository ships, and neither is a build of **this** repository: a reader who finds identifiers in either is reading the target-state case, not a contradiction of this record.
...
**Repository state, and the generated trees an in-place run leaves behind.** No tracked repository file was modified, added or deleted in producing this document. What a tracked-file check cannot show is recorded here rather than summarised as a clean tree: **restore and build runs performed against this checkout wrote eight ignored trees into it** — a `bin` and an `obj` beside each of the three projects, plus a restored `packages` tree under `src/MVC4` and another under `src/MVC5`. Their removal is **not** claimed here as a discharged act, and the omission is deliberate: §3.2's post-freeze run was performed in place as well and writes the same set again, so this is a **standing requirement** rather than one that can be closed once. **Whoever commits this assessment runs the two ignored-aware commands below and removes whatever they report**, because bare porcelain reports a clean tree either way — that is the check, and it is the committer's to discharge, not a state this appendix asserts. **The table below is that per-tree record**: each tree, the kind of run that writes it, and the ignore rule that hid it. Its extent column carries a figure for the two restored `packages` trees only, and **no extent is published for the six `bin` and `obj` trees** — for two independent reasons stated immediately after it.

| Tree an in-place run writes | Written by | The ignore rule that hid it | Extent |
| --- | --- | --- | --- |
| `src/MVC3/MvcMusicStore-Completed/MvcMusicStore/bin` | a compile | `[Bb]in/` [.gitignore:2] | not published — host- and path-dependent |
| `src/MVC3/MvcMusicStore-Completed/MvcMusicStore/obj` | a compile | `[Oo]bj/` [.gitignore:1] | not published — host- and path-dependent |
| `src/MVC4/MvcMusicStore/bin` | a compile | `[Bb]in/` [.gitignore:2] | not published — host- and path-dependent |
| `src/MVC4/MvcMusicStore/obj` | a compile | `[Oo]bj/` [.gitignore:1] | not published — host- and path-dependent |
| `src/MVC4/packages` | a restore | `Packages/` [.gitignore:33] — matches at any depth, and matches this lowercase directory only where the checkout is case-insensitive | **231** files, **29,937,044** bytes |
| `src/MVC5/MvcMusicStore/bin` | a compile | `[Bb]in/` [.gitignore:2] | not published — host- and path-dependent |
| `src/MVC5/MvcMusicStore/obj` | a compile | `[Oo]bj/` [.gitignore:1] | not published — host- and path-dependent |
| `src/MVC5/packages` | a restore | `Packages/` [.gitignore:33] — the same rule, under the same case dependency | **167** files, **48,979,685** bytes |
| **Restored payload — the two `packages` trees only, which is two of the eight and not a total for all eight** | two restores | `Packages/` [.gitignore:33] | **398** files, **78,916,729** bytes |

**Why those two figures are published and the six build-output extents are not.** The restored-payload figures are reproducible, and that is the whole of their claim to be here: they are what MVC 4's and MVC 5's committed `packages.config` manifests resolve to on disk, so a reader re-derives them by restoring those two solutions again into an empty tree and counting the two directories. **The condition is part of the figure and is stated once, here.** They hold for a restore by the **NuGet CLI in its default package-save mode, resolving these pins from `nuget.org`** — the form §3.2's `nuget restore … -NonInteractive` took. A different client, a different `PackageSaveMode` or a different effective source changes both the file count and the byte total, because what each package leaves on disk changes with all three; the repository declares no source of its own — §3.2 records that this one was inherited from host configuration, and the finding itself is [02](02-dependency-inventory.md) §6's — so the last of those is a property of the restoring host rather than of this checkout. Neither figure is a build outcome either: it records what a restore wrote, not that anything compiled (§1.4).

The six `bin` and `obj` extents fail that test twice over, and either reason alone would be enough to withhold them. **First, the state they described no longer exists and nothing immutable was kept of it** — the trees were removed and no per-file manifest, hash list or build log of them was retained, so a quoted figure would be a recollection with nothing a reader could check it against. **Second, a `bin`/`obj` extent is not a property of this repository at all**, so even a faithful replay of the documented flow legitimately reports a different figure rather than the same one twice. Two mechanisms account for that, both visible in the repository, and this document's own two build records already show the first of them at work:

- **A `bin` tree's extent follows how the host supplies the referenced assemblies, not what the project asks for.** MVC 3 references `System.Web.Mvc`, `System.Web.WebPages` and `System.Web.Helpers` with **no `HintPath` and no `<Private>false</Private>`** [src/MVC3/MvcMusicStore-Completed/MvcMusicStore/MvcMusicStore.csproj:42-44], which leaves the copy-local decision to the host: a host that resolves them from a directory outside the GAC copies them and their dependency closure into `bin`, a host that resolves them from the GAC copies none of them, and a host that has them nowhere leaves the references unresolved — which is exactly what §3.1's Mono attempt reports — and copies none either. One project, one command, three different `bin` extents.
- **An `obj` tree's extent follows where the checkout sits.** MSBuild writes its own per-project caches under `obj/Debug` — `MvcMusicStore.csproj.FileListAbsolute.txt` and `MvcMusicStore.csproj.AssemblyReference.cache` — and both record **absolute** paths and the reference set the host resolved, so their byte size moves with the checkout's location and with the host's assembly inventory even when the compiled output is identical.

Within one host at one path the figures are stable, so the accurate label is **host- and path-dependent**, not random — and that is precisely why they belong to a reader's own host rather than to this document.

**A reader who wants an extent measures their own checkout.** The three commands below report what that host and that path produced; each result is a **one-time observation of the checkout it was run in and is not evidence about this repository**, which is why no figure from them is published above:

```bash
git status --porcelain --ignored     # one line per ignored path, '!!' at the front
git ls-files --others --ignored --exclude-standard | wc -l
git ls-files --others --ignored --exclude-standard -z | xargs -0 -r stat -c %s \
  | awk '{s+=$1} END {print s+0}'
# On the delivered checkpoint: no output, 0 files, 0 bytes -- the cleared state the
# hygiene rule below requires. In a checkout where the documented restores have just
# run, the second and third lines given the pathspec
# '-- src/MVC4/packages src/MVC5/packages' report 398 and 78916729, and given no
# pathspec they add whatever that host and that path produced in the six bin/obj
# trees -- an observation of that checkout, not a figure this document adopts.
```

**The ignore-rule column is reproducible, and it does not name the rule a reader would guess.** `packages/*` [.gitignore:15] looks like the rule that covered the two restored package trees, and it did not cover them: a pattern with an interior separator is anchored to the directory holding the `.gitignore` — the repository root here — so it reaches a root-level `packages/` and nothing nested. The rule that matched them is `Packages/` [.gitignore:33], whose only separator is trailing, which matches a directory of that name at any depth. Deliverable 04 §A.6 reads these rules the same way, against the same paths. `--no-index` is required in the probe rather than merely tidy: without it `git check-ignore` exits 1 with no output for a **tracked** path whether or not a rule matches it, which is exactly how the committed payload at `src/MVC4/MvcMusicStore/packages` behaves under the plain form:
