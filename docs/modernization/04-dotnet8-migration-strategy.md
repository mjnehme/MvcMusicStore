...
| `.config/dotnet-tools.json` (§6.3) | [03](03-modernization-roadmap.md)'s **W6** — nothing before the conversion invokes `dotnet ef`, the manifest's one entry | **W10** consumes it; no later workstream adds a tool to it in this plan |
...
<a id="sql-cache-tool-withdrawal"></a>

**One entry, not two, and the absent one is a withdrawal rather than an oversight.** An earlier revision
of this section pinned `dotnet-sql-cache` `8.0.30` beside `dotnet-ef` on parallel reasoning:
`Microsoft.Extensions.Caching.SqlServer` reads and writes the session cache table but does not create it,
so some instrument must. **That pin is withdrawn, and the reason is the graph the tool package ships
rather than anything about what it does.**

- **What the packaged graph carries.** `dotnet-sql-cache` is packed with its dependencies as assemblies
  inside the tool package rather than resolving them from the project that installs it. At `8.0.30` its
  `tools/net8.0/any/dotnet-sql-cache.deps.json` declares **51 libraries** and the installed payload is
  **35 DLLs**. Among them: `System.Drawing.Common` `5.0.0`, carrying the CRITICAL
  **GHSA-rxg9-xrhp-64gj** / **CVE-2021-24112**, fixed in `5.0.3`; and the six `Microsoft.IdentityModel.*`
  assemblies together with `System.IdentityModel.Tokens.Jwt` at `6.8.0`, carrying the MODERATE
  **GHSA-59j7-ghrg-fj52**, fixed in `6.34.0`. `System.Security.Permissions` `5.0.0`,
  `System.Windows.Extensions` `5.0.0` and `Microsoft.Data.SqlClient` `4.0.5` ship alongside them.
- **Why no `8.0.x` release fixes it.** `8.0.30` is the **highest** release published on the `8.0` line —
  the `8.x` set ends `8.0.25, 8.0.26, 8.0.27, 8.0.28, 8.0.29, 8.0.30` — so there is no later `8.0` patch
  to advance to, and this is the release the band of §7.2 would otherwise select.
- **Why a version override does not fix it either.** A `PackageReference` override or a lockfile
  constraint changes what a **project** resolves. It does not change the assemblies a **tool package
  ships**, because those were selected when the tool was packed. The vulnerable files are physically in
  the payload, and no consumer-side constraint reaches them.
- **Why the advisory-clear release is not adoptable in this position.** `10.0.11` **is** clean — 30
  libraries, no `System.Drawing.Common`, `Microsoft.Data.SqlClient` `6.1.1`,
  `Microsoft.IdentityModel.*` `7.7.1` — but it is a .NET 10 tool and requires a .NET 10 runtime in the
  image that runs it. §6.2's `global.json` pins the SDK feature band at `8.0.400` with
  `rollForward: latestPatch`, and the executor that would run the tool is the privileged migration image
  of [06](06-azure-hosting-recommendations.md) §12.7.2.1. Admitting a second runtime into that image to
  create one table is a larger change to its contract than the table warrants.
- **What replaces it, and why nothing is lost.** The DDL the tool emits is **byte-identical** across
  `8.0.30` and `10.0.11`, and its `script` subcommand emits that DDL with an `--idempotent` guard. The
  output is therefore captured **once** at authoring time, reviewed, and committed as
  `deploy/sql/SessionCache.sql` (§12.2);
  the release executes it with the T-SQL client the migration executor already carries for exactly this
  class of work ([06](06-azure-hosting-recommendations.md) §6.10.1). That is already how
  [06](06-azure-hosting-recommendations.md) §6.4's **`M2`** creates `dbo.SecurityAuditLog`, so the session
  cache table becomes the second object created that way rather than the sole exception to it.
  [06](06-azure-hosting-recommendations.md) §6.4 owns the DDL text, its review record and its provenance.
- **What the target tool graph contains as a result.** One tool — `dotnet-ef` `8.0.30` — whose packaged
  graph carries no advisory. §7.4 records the one target-state advisory this plan does own, and it is a
  transitive package pin rather than a tool payload.

The distinction the withdrawn row was drawing still holds and is still load-bearing: **`Caching.SqlServer`
is the runtime provider and creates nothing.** What changed is the instrument that creates the table, not
the fact that one is required. A plan that lists only the runtime packages still produces a deployment with
a migration command nobody can invoke and a session cache with no table behind it. The table's schema and
name, the principal that executes its DDL and the point in the release at which that happens are
[06](06-azure-hosting-recommendations.md)'s to specify; the *pin* — and now this *withdrawal* — is here.
...
The band holds **nine** entries rather than the ten an earlier revision counted, and the difference is the
withdrawal recorded in §6.3: `dotnet-sql-cache` `8.0.30` was a band member until its packaged graph was
audited, and it is no longer pinned, restored or shipped anywhere in the target. Both consequences above
range over the nine that remain.
...
| nuget.org | `Microsoft.Extensions.Identity.Core` | `8.0.30` | **The assembly that carries `ILookupNormalizer` and the framework's default `UpperInvariantLookupNormalizer`, which the suite resolves and calls rather than reimplementing, so its diagnostic pseudonyms canonicalize by the application's own rule.** Declared in `src/MvcMusicStore.Contracts.Tests` **and only there** (§12.2, §12.3); the web application declares it nowhere and needs no reference, because it reaches the same assembly transitively through the row above. It is a **band** entry rather than an independent one for the correctness reason §7.8 states — the normalizer the suite calls must be the one the application's Identity stack resolves. Registry facts: `8.0.30` listed, **not deprecated**, licensed **MIT**, published 11 August 2026 — the band's own release date — and publishing a `net8.0` asset alongside `net462` and `netstandard2.0` [NuGet Gallery, *Microsoft.Extensions.Identity.Core*, <https://www.nuget.org/packages/Microsoft.Extensions.Identity.Core> — verified 2026-08-29] |
...
**A seventh pin is declared only by a test project and is on the band nonetheless**, which is worth
stating because a reader counting test-declared rows finds seven where the paragraph above counts six.
The six above are the test-declared pins that version **independently**;
`Microsoft.Extensions.Identity.Core` `8.0.30` is a test-declared pin that does not, because it is a
Microsoft-shipped .NET 8 package whose version has to equal the one the application's own Identity stack
resolves. It is therefore counted among the band's **nine** entries and moves under §7.1's two
consequences rather than on a cadence of its own (§7.8).
...
abstractions pin added last, on 2026-08-29, with §7.8 recording it. All nine `8.0.30`
...
that one.** The sentence above is about advisories and claims nothing more. **Four of the rows in this
table resolve a deprecated package between them**, and the count is what a restore reports rather than
what a per-pin registry read implies: the closure `Microsoft.Data.SqlClient` `5.1.9` brings carries
**eleven** — `Azure.Identity` `1.12.1`, MSAL `4.76.0` and nine others, one of them marked for a named
defect; `Microsoft.EntityFrameworkCore.Design` `8.0.30` reaches `System.Collections.Immutable` `6.0.0`
through the Roslyn workspace packages its design-time services load;
`Microsoft.Playwright` `1.62.0` raises `System.Text.Json` to `6.0.10`, which carries the marker at that
version as well as at the closure's `4.7.2`; and `xunit` `2.9.2` brings **four** v2 subpackages under its
own line's deprecation (§7.3). **§7.6 is that disclosure's single home** for all four: it states each
closure and each reason string, why each pin is nonetheless taken, which surface each residual is actually
on, and the override floor a reader who declines one can apply. Nothing about any of it changes a version
in this table.
...
Nothing else in §7.2 is affected. The **seven** other pins a test project declares were checked and none
is flagged: `Microsoft.AspNetCore.Mvc.Testing` `8.0.30`, `xunit.runner.visualstudio` `2.8.2`,
`Microsoft.NET.Test.Sdk` `17.11.1`, `AngleSharp` `1.7.2`, `Microsoft.Data.SqlClient` `5.1.9`,
`Microsoft.Playwright` `1.62.0` and `Microsoft.Extensions.Identity.Core` `8.0.30` are all listed and
**not** deprecated, so none joins this gate — and that is asserted mechanically rather than read pin by
pin: an isolated restore of the whole test pin set reports **this** pin as its only deprecated *top-level*
package (§7.6.1). All seven nonetheless fall under the ordinary re-verification of §7.1 consequence 1: the
in-process host and the Identity abstractions move with the band, and the two runner pins have advanced
since the plan was written.

**That statement is about the eight pins themselves and does not extend to what they resolve — where the
deprecated set is larger than this pin and larger than one closure.** A package's own registry record
carries no claim about its dependencies, so the disclosure is a list and not an exception. **This pin's
own line accounts for four of the resolved members**: `xunit` `2.9.2` is composed of `xunit.assert` and
`xunit.core`, and `xunit.core` of `xunit.extensibility.core` and `xunit.extensibility.execution`, all
`2.9.2` and all carrying the same v2 `Legacy` marker with an `xunit.v3.*` alternate — so the gate above
settles **five** rows rather than one, and a dependency policy that rejects deprecated packages fails on
all five. **Three further roots resolve a deprecated member**, none of them this pin's:
`Microsoft.Data.SqlClient` `5.1.9`'s eleven transitives, including MSAL `4.76.0` under the `CriticalBugs`
reason; `Microsoft.EntityFrameworkCore.Design` `8.0.30`'s `System.Collections.Immutable` `6.0.0`; and
`Microsoft.Playwright` `1.62.0`'s raise of `System.Text.Json` to `6.0.10`. **§7.6 records every one of
them, with each reason, the reason each pin still stands, and the override floor where one exists** — and
the decision it hands over belongs to the same owners as this gate, engineering jointly with the plan
owner, at the same moment, so all of it is settled together rather than separately.
...
applies here, and **the disclosure for every deprecated member of §7.2's resolved graphs lives in this
section** — this pin's closure first, because it is the largest, and then the three other roots that
resolve one, in the block that closes this section. §7.2 and §7.3 point here rather than restating any of
it.

`Microsoft.Data.SqlClient` `5.1.9`'s `net6.0` dependency group — the group a `net8.0` project consumes,
per the paragraph above — declares **thirteen** packages, and the closure behind them is **39 distinct
transitive packages** at the versions a restore resolves absent any other reference that raises them.
**Eleven of the 39 carry a deprecation record on nuget.org. None of the 39 carries a vulnerability
record.** The 39 is **measured rather than counted by hand**, which matters because a hand walk of
declarations is exactly the method that miscounts a diamond: an isolated `net8.0` project whose only
`PackageReference` is this pin restores to **40 library entries** in its `project.assets.json` — the pin
plus 39 — and `dotnet list package --include-transitive` prints the same 39 [NuGet, *NuGet V3 API —
registration and package content resources*, <https://api.nuget.org/v3/index.json> — every entry's
registration leaf read, verified 2026-09-01; the closure re-measured by isolated restore with
`dotnet list package --include-transitive`, `--deprecated --include-transitive` and
`--vulnerable --include-transitive` on 2026-09-04, which is where the 39, the eleven and the empty
vulnerability result come from]. Those checks are later than the 2026-08-29 checks recorded above, and
they are checks of the *closure* rather than of the pin, which is why their dates differ.
...
#### 7.6.1 The rest of the resolved graphs — three further roots, disclosed here because this section is the single home

**This pin's closure is the largest deprecated set in the target, and it is not the only one — so the
"one unclean closure" framing is withdrawn by name.** A per-pin registry check answers a question about
pins; the question a reviewer actually asks is what a restore *resolves*, and the answer is produced by
restoring rather than by reading. Restored in isolation on 2026-09-04, at exactly §7.2's versions, the
target's two graphs report this:

| Graph restored | Root pins in it | `dotnet list package --deprecated --include-transitive` |
| --- | --- | --- |
| **Application** — the six pins the web application declares (§10.4) | `Microsoft.EntityFrameworkCore`, `.SqlServer`, `.Design`, `Microsoft.AspNetCore.Identity.EntityFrameworkCore`, `Microsoft.Extensions.Caching.SqlServer`, `Microsoft.AspNetCore.DataProtection.EntityFrameworkCore`, all `8.0.30` | **89 library entries, twelve deprecated** — this section's eleven, plus `System.Collections.Immutable` `6.0.0` |
| **Test** — the pins the two test projects declare (§12.2) | `Microsoft.AspNetCore.Mvc.Testing` `8.0.30`, `xunit` `2.9.2`, `xunit.runner.visualstudio` `2.8.2`, `Microsoft.NET.Test.Sdk` `17.11.1`, `AngleSharp` `1.7.2`, `Microsoft.Data.SqlClient` `5.1.9`, `Microsoft.Playwright` `1.62.0`, `Microsoft.Extensions.Identity.Core` `8.0.30` | **92 library entries, sixteen deprecated** — the `xunit` pin itself (§7.3), ten of this section's eleven, `System.Text.Json` at **`6.0.10`** rather than `4.7.2`, and **four** `xunit` v2 subpackages |

**Neither graph carries a vulnerability record**: `dotnet list package --vulnerable --include-transitive`
reports *"no vulnerable packages"* for the application graph, for the test graph and for this pin's closure
alone, which is the mechanically asserted form of §7.2's advisory statement rather than an inference from
one of them.

**Every deprecated resolved member, by its owning root.** Four roots, `11 + 1 + 1 + 4 = 17` `(package,
version)` pairs over **sixteen** distinct package identifiers — sixteen and not seventeen because
`System.Text.Json` appears twice, at two versions, for the reason its row gives:

| Owning root | Deprecated members it resolves | Reason, and the path that reaches it |
| --- | --- | --- |
| `Microsoft.Data.SqlClient` `5.1.9` | **Eleven** — the table above | `CriticalBugs` on MSAL, `Other` on the two rows that inherit it, `Legacy` on the seven-package IdentityModel `6.x` line and on `System.Text.Json` `4.7.2`. Declared by the driver itself except for `Microsoft.Identity.Client.Extensions.Msal`, four of the seven IdentityModel packages and `System.Text.Json`, each of which is reached one level further on — the table above gives every path |
| `Microsoft.EntityFrameworkCore.Design` `8.0.30` | **One** — `System.Collections.Immutable` `6.0.0` | `Legacy`, message *"This package has been deprecated as part of the .NET Package Deprecation effort"*, pointing at `dotnet/core/issues/10347`, with **no alternate package named**; published 8 November 2021. The path is the Roslyn workspace chain the design-time services load: `Design` → `Microsoft.CodeAnalysis.CSharp.Workspaces` `4.5.0` → `Microsoft.CodeAnalysis.Common` `4.5.0` → this package (`System.Reflection.Metadata` `6.0.1`, itself on that chain, declares it too) |
| `Microsoft.Playwright` `1.62.0` | **One** — `System.Text.Json` `6.0.10` | `Legacy`, the same .NET Package Deprecation message and issue, no alternate package; published 8 October 2024. **This is a raised resolution rather than a new identifier**: the harness declares `6.0.10` while every other declarer in the graph declares `4.7.2` or lower, so the test graph resolves `6.0.10` — a floor raised above a deprecated version onto another deprecated version, which is why raising a floor is not by itself a remedy here |
| `xunit` `2.9.2` | **Four** — `xunit.assert`, `xunit.core`, `xunit.extensibility.core` and `xunit.extensibility.execution`, all `2.9.2` | `Legacy`, each with an `xunit.v3.*` alternate named and the v2 line's own message *"This package will only be updated for security issues. All future feature work has moved onto v3."* They are the metapackage's own composition — `xunit` → `xunit.assert` and `xunit.core`, and `xunit.core` → the two extensibility packages — so the deprecation is §7.3's decision reaching one level down and not a second finding. `xunit.analyzers` `1.16.0` and `xunit.abstractions` `2.0.3`, also in that closure, carry **no** deprecation record |

Reason strings, messages, alternates and publication dates are the registry's own, read from each
version's registration leaf on the date of the measurement [NuGet Gallery,
*System.Collections.Immutable*, <https://www.nuget.org/packages/System.Collections.Immutable>;
NuGet Gallery, *System.Text.Json*, <https://www.nuget.org/packages/System.Text.Json>;
NuGet Gallery, *xunit.core*, <https://www.nuget.org/packages/xunit.core> — all verified 2026-09-04].

**What each of the three adds to the position this section already takes, stated so none of them is
either inflated or waved through.** All three are `Legacy` markers rather than defect reports, so none
raises a floor of §7.4's kind and none is a precondition on anything:

- **`System.Collections.Immutable` `6.0.0` is on a design-time path, not a request path.**
  `Microsoft.EntityFrameworkCore.Design` is loaded by the migration executor and by `dotnet ef`, not by
  the running application (§6.3), so the residual sits on a tool the release pipeline runs rather than on
  the connection path the `CriticalBugs` row sits on. It cannot be moved by this document for §7.1's
  reason: the version is what the band's own `Design` package resolves, and a direct override would raise
  a Roslyn-line dependency inside the tool that generates migrations.
- **`System.Text.Json` `6.0.10` is a test-graph resolution and the application never sees it.** The web
  application resolves `4.7.2` from the closure above; the raise happens only where
  `Microsoft.Playwright` is declared, which is the two test projects and neither production project
  (§7.7). It is the one member of the four roots that a *fixture* pin introduces at a version the
  application does not deploy — which is the asymmetry the `5.1.9` derivation above exists to avoid — and it is
  accepted because the alternative is dropping the browser harness that covers the one scripted flow,
  not because the version is preferable.
- **The four `xunit` subpackages add no decision of their own.** They resolve because the `xunit`
  metapackage is composed of them, so the gate that settles §7.3's pin settles these four with it: a
  dependency policy that rejects NuGet-deprecated packages fails on five rows rather than one, and the
  same policy exception or the same move to `xunit.v3` clears all five.

**Owner and timing are §7.3's, deliberately, so that this block adds no decision of its own.** Engineering
jointly with the plan owner, at the moment §7.1 consequence 1 re-verifies the band — because that is when
every version above is re-read anyway, and because three of the four roots are versions the plan fixed.
The one action this block does require is procedural rather than technical: **the deprecated set is
re-measured by restore at that moment, not re-remembered**, since a member can be added or cleared by a
publisher without any pin in §7.2 changing.
...
### 7.8 `Microsoft.Extensions.Identity.Core` `8.0.30` — the normalizer the suite invokes, and why its assembly is a band pin rather than an independent one
...
- **Tooling availability, and the tool is pinned like every other executable this document recommends.**
  Library Manager is available through Visual Studio's own tooling and, outside the IDE, through the
  `Microsoft.Web.LibraryManager.Cli` tool — so the update action is not IDE-dependent. Outside the IDE it
  is installed as **`dotnet tool install -g Microsoft.Web.LibraryManager.Cli --version 3.0.114`**, never
  the bare form: an unversioned install resolves to whatever the feed serves that day, and this is the
  tool that decides which *bytes* of four browser libraries get committed, so leaving its own version open
  would put an unbounded executable at the head of the client-asset supply chain. Three properties were
  measured on the artifact at `3.0.114`, the current release. It targets **`net8.0`**, so unlike the
  assessment tool of [§11.1](#appcat-pin) it runs under the band `global.json` already pins and needs no
  additional runtime. Its declared graph is **nine libraries** — six packages and three of its own
  projects — resolving to 44 assemblies across 65 files, which is a small enough surface to audit rather
  than merely trust. And an advisory audit of that graph at `low` severity reports **nothing**: no
  `NU1901`–`NU1904` at any level. That is the cleanest supply-chain result of any tool this corpus
  recommends, and it is recorded as a measured value so that moving the pin obliges re-measuring rather
  than assuming the result carries forward.
- **It is still not in `.config/dotnet-tools.json`, for the reason the bullet above already gives.**
  `libman restore` is a developer-initiated action whose *output* is committed, so no build and no release
  ever invokes this tool; §6.3's manifest is the set of tools a release must be able to restore, and an
  entry here would make a client-asset refresh tool a deployment prerequisite. Pinned, and outside the
  manifest — the same posture as [§11.1](#appcat-pin), reached for the same reason.
...
### 9.4 The one `libman.json` contract — provider ids, destinations and file lists
...
entries** and it selects **thirteen files** — `3 + 4 + 2 + 4`, because not one entry selects a single
file — and no other section restates its `files` lists: §9.5 reconciles these same thirteen selections
against the provider and the served paths and states the rule that selects each one, §9.6 states the order
the rendered ones load in, and §9.7 carries the provider coordinates and the identifier trap without a
second file list. A manifest
...
**The thirteen files it acquires, named once so nothing downstream has to infer them.** Four entries,
thirteen files — jQuery three, jQuery Validation four, the unobtrusive adapter two and Bootstrap four, so
`3 + 4 + 2 + 4 = 13` — and the three roles below are what make the number thirteen rather than four or
six. §9.5 states the rule that puts each file in one of them.

- **Six are referenced by a rendered element outside Development** —
  `wwwroot/lib/jquery/jquery.min.js`, `wwwroot/lib/jquery-validation/jquery.validate.min.js`,
  `wwwroot/lib/jquery-validation/additional-methods.min.js`,
  `wwwroot/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.min.js`,
  `wwwroot/lib/bootstrap/css/bootstrap.min.css` and
  `wwwroot/lib/bootstrap/js/bootstrap.bundle.min.js`. **Which pages reference `additional-methods` is the
  one selection question this manifest leaves to another section** —
  [05 §8.1.3](05-aspnet-core-migration-approach.md)'s, which fixes its reference site, and which is why
  §9.6's load-order table names five library files rather than six.
- **Four are the unminified Development-time builds of the four jQuery-family scripts** —
  `wwwroot/lib/jquery/jquery.js`, `wwwroot/lib/jquery-validation/jquery.validate.js`,
  `wwwroot/lib/jquery-validation/additional-methods.js` and
  `wwwroot/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.js`. The elements that choose
  between a pair are [05 §8.1.3](05-aspnet-core-migration-approach.md)'s as well; Bootstrap has no pair,
  for the reason §9.5 gives.
- **Three are source maps** — `wwwroot/lib/jquery/jquery.min.map`,
  `wwwroot/lib/bootstrap/css/bootstrap.min.css.map` and
  `wwwroot/lib/bootstrap/js/bootstrap.bundle.min.js.map`. Three and not six, because the two validation
  libraries publish no map at all at these versions (§9.5).

`6 + 4 + 3 = 13` from **four** entries, which is the figure §9.3's third count states and the one
[05](05-aspnet-core-migration-approach.md) §8.1.1 adds to the application's own relocated and new files
when it states the served payload.

**The six details in it that are decisions rather than transcription.**
...
- **`jquery-validate`, not `jquery-validation`.** cdnjs publishes jQuery Validation under the id
  ...
  other by pattern.
- **The four entries are exactly §9.2's four retained libraries, at exactly §9.2's versions** — jQuery
...
- **Every entry states its `files` explicitly, and the selection rule has three clauses rather than
  one.** The default is to vendor the library's *entire* published file set — for `bootstrap@5.3.3` that
  is **136 published files**, including RTL builds, unminified builds, per-component ESM bundles and their
  maps, all of which would then be **committed**, because §9.3 commits the output. Explicit selection is
  what keeps a committed vendored directory reviewable. **A file is selected if it is rendered outside
  Development, or if it is the Development-time unminified build of a rendered script, or if it is a
  source map of a file this manifest already acquires and the provider publishes one** — §9.5 states each
  clause with the reason it exists. The third clause is the one worth spelling out here, because the
  earlier rule this document applied was "vendored only if served" and that rule does not describe this
  manifest: a map is useful precisely beside the source it references, and the first two clauses have
  already committed both builds of every jQuery-family file, so the maps cost **three** committed files
  and buy developer tools that resolve a minified library without an edit to this manifest. Three and not
  six is a provider fact rather than a preference: jQuery publishes a map for its minified build and
  Bootstrap publishes one for each of the two compiled builds selected above, while
  `jquery-validate@1.21.0` and `jquery-validation-unobtrusive@4.0.0` publish **no `.map` file at all** at
  these versions, so there is none to select. **One spelling trap is therefore load-bearing rather than
  hypothetical, because this manifest selects the file:** jQuery's map is published as
  **`jquery.min.map`**, not `jquery.min.js.map`, at this version — the provider's index lists six files
  for `jquery@3.7.1` and no `jquery.min.js.map` among them — so a line copied from another project's
  manifest, where the `.min.js.map` convention is near-universal, fails to restore on exactly that one
  line [cdnjs API, <https://api.cdnjs.com/libraries/jquery/3.7.1> and
  <https://api.cdnjs.com/libraries/bootstrap/5.3.3> — both verified 2026-09-04].
...
in that entry's `files` list — **thirteen** names in total across the four entries. All four entries above
were checked that way and all thirteen names are published at the pinned versions [cdnjs API,
<https://api.cdnjs.com/libraries/jquery/3.7.1>,
<https://api.cdnjs.com/libraries/jquery-validate/1.21.0>,
<https://api.cdnjs.com/libraries/jquery-validation-unobtrusive/4.0.0> and
<https://api.cdnjs.com/libraries/bootstrap/5.3.3> — all verified 2026-09-04]; `Respond` and `Modernizr`
have no entry because §9.2 removes them.

---

### 9.5 The manifest reconciled — provider, destination, file and served path

§9.4 holds the manifest; this section reconciles the **same** four entries and thirteen selections against
the provider that serves them and against the URLs the browser then requests, states the rule that selects
each file, and adds nothing to the file list. It carries no second copy of the JSON, for the reason §9.4
states: a manifest quoted twice is two manifests as soon as one copy is edited.
...
**The inventory the manifest generates: thirteen files, and the arithmetic that produces the thirteen.**
The entry count and the file count are different numbers, which is why both are stated rather than one
being quoted loosely: jQuery selects **three**, jQuery Validation **four**, the unobtrusive adapter **two**
and Bootstrap **four**, so `3 + 4 + 2 + 4 = 13` from **four** entries. Those thirteen files are the
committed output §9.3 requires, they are the whole of the `wwwroot/lib/**` row in §12.2's map, and thirteen
is the figure [05](05-aspnet-core-migration-approach.md) §8.1.1 adds to the application's own relocated and
new files when it states the served payload. Every row below is one file — its committed path, the entry
that selects it, the clause of the rule that selects it, and the URL the browser then requests. The served
path is the destination with `wwwroot` removed, because `wwwroot` is the web root and never appears in a
URL, which is the distinction [05](05-aspnet-core-migration-approach.md) §12.7 asserts on.

**The three clauses of the selection rule, each with the reason it exists rather than the shape it has.**
Together they partition the thirteen as `6 + 4 + 3`, and no file satisfies two clauses:

- **Rendered outside Development — six.** The file an element references in a deployed response. That is
  the whole of what a "vendored only if served" rule would select, and on its own it would leave the next
  two clauses' seven files unacquired.
- **The Development-time unminified build of a rendered script — four.** `@Scripts.Render` emits a
  bundle's members **unminified** while `<compilation debug="true">` is in force
  [src/MVC5/MvcMusicStore/Web.config:33] and the single minified bundle once the Release transform has
  removed that attribute [src/MVC5/MvcMusicStore/Web.Release.config:18]. The target has no bundler and
  minifies nothing, so that debug-versus-release behaviour survives **only** as a choice between two
  already-acquired files — which is why both builds of each of the four jQuery-family scripts are
  acquired, and why omitting the pair would drop a behaviour the source already has. **The elements that
  choose between a pair are [05 §8.1.3](05-aspnet-core-migration-approach.md)'s**, and Bootstrap has no
  pair because nothing references its sources: only its compiled builds are acquired, so an element
  wrapped around it would offer a choice between one file and itself.
- **A source map of an acquired file, where the provider publishes one — three.** jQuery publishes
  `jquery.min.map` and Bootstrap publishes a map for each of the two compiled builds selected here;
  `jquery-validate@1.21.0` and `jquery-validation-unobtrusive@4.0.0` publish none at these versions, so
  the clause selects three files and not six [cdnjs API,
  <https://api.cdnjs.com/libraries/jquery/3.7.1>,
  <https://api.cdnjs.com/libraries/jquery-validate/1.21.0>,
  <https://api.cdnjs.com/libraries/jquery-validation-unobtrusive/4.0.0> and
  <https://api.cdnjs.com/libraries/bootstrap/5.3.3> — all verified 2026-09-04].

**One acquired file is rendered by a page rather than by the layout, and referencing only its neighbour is
a silent-non-running failure.** `additional-methods.min.js` is the second half of the validation plugin's
distribution at this version, and a page that references `jquery.validate.min.js` alone still renders and
still validates on the server — so a rule whose client half lives in the second file fails only in the
browser, and surfaces as a validation message that appears after a round trip instead of on the field.
That is the same failure shape as selecting `bootstrap.min.js` over the bundle, one library along. **Which
pages reference it is [05 §8.1.3](05-aspnet-core-migration-approach.md)'s**, stated there so that no
acquired file is committed, published and referenced by nothing.

| Committed file | Selected by | Clause | Served path |
| --- | --- | --- | --- |
| `src/MvcMusicStore/wwwroot/lib/jquery/jquery.min.js` | `jquery@3.7.1` | Rendered | `/lib/jquery/jquery.min.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery/jquery.js` | `jquery@3.7.1` | Development build | `/lib/jquery/jquery.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery/jquery.min.map` | `jquery@3.7.1` | Source map | `/lib/jquery/jquery.min.map` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation/jquery.validate.min.js` | `jquery-validate@1.21.0` — the cdnjs id for library **jquery-validation** | Rendered | `/lib/jquery-validation/jquery.validate.min.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation/jquery.validate.js` | `jquery-validate@1.21.0` | Development build | `/lib/jquery-validation/jquery.validate.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation/additional-methods.min.js` | `jquery-validate@1.21.0` | Rendered — by the form pages [05 §8.1.3](05-aspnet-core-migration-approach.md), not by the layout | `/lib/jquery-validation/additional-methods.min.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation/additional-methods.js` | `jquery-validate@1.21.0` | Development build | `/lib/jquery-validation/additional-methods.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.min.js` | `jquery-validation-unobtrusive@4.0.0` | Rendered | `/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.min.js` |
| `src/MvcMusicStore/wwwroot/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.js` | `jquery-validation-unobtrusive@4.0.0` | Development build | `/lib/jquery-validation-unobtrusive/jquery.validate.unobtrusive.js` |
| `src/MvcMusicStore/wwwroot/lib/bootstrap/css/bootstrap.min.css` | `bootstrap@5.3.3` | Rendered | `/lib/bootstrap/css/bootstrap.min.css` |
| `src/MvcMusicStore/wwwroot/lib/bootstrap/css/bootstrap.min.css.map` | `bootstrap@5.3.3` | Source map | `/lib/bootstrap/css/bootstrap.min.css.map` |
| `src/MvcMusicStore/wwwroot/lib/bootstrap/js/bootstrap.bundle.min.js` | `bootstrap@5.3.3` | Rendered | `/lib/bootstrap/js/bootstrap.bundle.min.js` |
| `src/MvcMusicStore/wwwroot/lib/bootstrap/js/bootstrap.bundle.min.js.map` | `bootstrap@5.3.3` | Source map | `/lib/bootstrap/js/bootstrap.bundle.min.js.map` |

**Thirteen rows: six rendered, four Development builds, three source maps** — the `6 + 4 + 3` the clauses
above partition, and the same thirteen the manifest's four `files` lists name.

| Provider identifier and version | Destination | `files` | Served paths |
| --- | --- | --- | --- |
| `jquery@3.7.1` | `wwwroot/lib/jquery/` | `jquery.js`, `jquery.min.js`, `jquery.min.map` | `/lib/jquery/` + each name |
| `jquery-validate@1.21.0` — library **jquery-validation** | `wwwroot/lib/jquery-validation/` | `jquery.validate.js`, `jquery.validate.min.js`, `additional-methods.js`, `additional-methods.min.js` | `/lib/jquery-validation/` + each name |
| `jquery-validation-unobtrusive@4.0.0` | `wwwroot/lib/jquery-validation-unobtrusive/` | `jquery.validate.unobtrusive.js`, `jquery.validate.unobtrusive.min.js` | `/lib/jquery-validation-unobtrusive/` + each name |
| `bootstrap@5.3.3` | `wwwroot/lib/bootstrap/` | `css/bootstrap.min.css`, `css/bootstrap.min.css.map`, `js/bootstrap.bundle.min.js`, `js/bootstrap.bundle.min.js.map` | `/lib/bootstrap/` + each name, path within the destination preserved |
...
placement in the views; this table is the order it renders them in.

**The manifest acquires thirteen files and this table covers the rendered subset**, which is the one place
the two counts have to be told apart: a load order is a property of the elements a response carries, so the
table names the file each element references and nothing else. Three consequences, each of them
[05 §8.1.3](05-aspnet-core-migration-approach.md)'s to render:

- **Five library files appear below rather than §9.4's six rendered ones.**
  `additional-methods.min.js` is deliberately absent: it travels inside the validation library beside its
  own core file, and its reference site — all six ported form pages, between the core plugin and the
  unobtrusive adapter — is [05 §8.1.3](05-aspnet-core-migration-approach.md)'s, which states it for that
  reason. Its ordering constraint is the second reason below, applied one file along.
- **The four unminified Development builds add no position.** Each is named by the
  `<environment include="Development">` half of the same element whose
  `<environment exclude="Development">` half names the minified file, in the same position and the same
  order, which is what makes one load order hold in both environments.
- **The three source maps have no position at all**, because no element references a map: a browser
  requests one only when developer tools ask for it.
...
registration 05 has since replaced, and removing it leaves the **web application's** six pins as exactly
the ones the plan fixes for it. That is the claim, stated at the application's scope rather than the
table's: §7.2 carries five further pins the plan's §0.5.2 does not — the response-body parser, the SQL
client, the browser harness and the Identity abstractions, each with its own deviation record in §7.5
through §7.8, and the operator tool's logging provider, whose off-band status and single declaring project
that pin's own row states — and **not one of the five is declared by the web application**.
...
tool** and it is invoked as `appcat analyze <application-path>`, producing a report in HTML, JSON or CSV
...
**It is pinned to an exact version, because "install the tool" is not a pin and this document pins every
executable it recommends.** The install is
`dotnet tool install -g dotnet-appcat --version 1.0.1127` — never the bare form, which resolves to
whatever the feed serves on the day and makes two engineers' assessments of one commit incomparable. The
pin is `1.0.1127`, the current release, and four properties of it were measured on the artifact rather
than read off a listing:

| Property | Measured value | Why it is recorded |
| --- | --- | --- |
| **Tool target framework** | **`net10.0`** — the tool's payload sits under `tools/net10.0/any` | This is the consequential one. `global.json` pins the SDK feature band at **`8.0.400`** (§6.1), so a machine provisioned only to that band **cannot run this tool at all**. The assessment workstation therefore needs a .NET 10 runtime in addition to the pinned SDK, and that is a prerequisite of the *assessment*, never of the build or the release |
| **Declared graph** | **82 declared libraries**, 75 of them `package`-type | The surface a software-composition question is asked against, stated as a number so a later answer is checkable |
| **Installed payload** | **905 assemblies** across 970 files | Larger than the declared graph, because the tool carries analyzer and runtime payloads its metadata does not enumerate one-for-one |
| **Advisory posture** | **Four identifiers in the declared graph, none of them in the shipped payload** | Recorded in full below, because a partial statement here would read as either an all-clear or a blocker and it is neither |

**The advisory result, stated precisely, because the imprecise version of it is misleading in both
directions.** An audit of the declared graph at `low` reports four identifiers: `NuGet.Packaging` `6.14.0`
at **low** severity (`GHSA-g4vj-cjjj-v7hg`), and `System.Formats.Asn1` `6.0.0`
(`GHSA-447r-wph3-92pm`), `System.Net.Http` `4.3.0` (`GHSA-7jgj-8wvc-jh57`) and
`System.Text.RegularExpressions` `4.3.0` (`GHSA-cmhx-cq75-c4mj`) at **high**. **None of those four
assemblies is present in the installed tool** — a scan of all 905 shipped assemblies finds no copy of any
of them. The three `4.3.0`/`6.0.0` entries are the familiar compatibility-shim references a modern target
framework resolves to the platform, and the `NuGet.Packaging` entry is declared metadata whose assembly
the tool does not carry. So the honest statement is that the advisories are **declared rather than
packaged**, and this is the same distinction — declared graph versus shipped bytes — that decided the
withdrawal in [§6.3](#sql-cache-tool-withdrawal), applied here and reaching the opposite conclusion
because the measurement came back the other way. Two consequences follow. The pin is **admissible**: an
engineer's assessment workstation may install it. And it is admissible **on this evidence rather than on
the absence of a finding**, so the same measurement is re-run when the pin is moved, on the artifact
rather than on the listing, and the result recorded here beside the version. That re-measurement is a
condition of moving the pin rather than a suggestion attached to it.
...
`.config/dotnet-tools.json` alongside `dotnet-ef` — that entry is *required to
...
**And a second, independent reason makes the exclusion mechanical rather than editorial:** the pin above
targets `net10.0`, while §6.3's manifest is restored inside an image built on a .NET 8 SDK base whose band
`global.json` fixes at `8.0.400` — so an entry for it there would not merely be inappropriate, it would
**fail the restore**. Being pinned and being in the manifest are therefore separate questions, and this
tool is pinned without being in it.
...
agent instead"* — and directs a reader to that agent
[Microsoft Learn, *.NET Upgrade Assistant overview*,
<https://learn.microsoft.com/dotnet/core/porting/upgrade-assistant-overview> — verified 2026-08-28,
re-verified 2026-09-04]. That page is cited here for the **deprecation status and nothing else**, which is
what this section exists to record. It is recorded because a strategy document is exactly where a reader
expects to find "run the Upgrade Assistant" as step one, and a plan that recommended it without recording
its deprecation would be pointing at unsupported tooling.

**The successor's prerequisite is taken from the successor's own page, because the deprecation notice is a
patch behind it.** The dedicated installation page requires **Visual Studio 2026, or Visual Studio 2022
version 17.14.17 or later** — *"Visual Studio 2026 (or Visual Studio 2022 version 17.14.17+)"* — together
with the .NET desktop development workload carrying the **GitHub Copilot** and **GitHub Copilot app
modernization** optional components, and a GitHub Copilot subscription, paid or free
[Microsoft Learn, *Install GitHub Copilot upgrade*,
<https://learn.microsoft.com/dotnet/core/porting/github-copilot-upgrade/install> — page dated 2026-07-07,
last updated 2026-08-04, verified 2026-09-04]. **`17.14.17` is therefore the minimum this document
requires.** The Upgrade Assistant overview quoted above continues to name a minimum **one patch lower**,
and the two are not reconciled by taking the smaller: a prerequisite belongs to the tool that has it, and the
deprecation notice on a retired tool's page is not revised on the successor's cadence. Anyone installing
the successor reads the installation page, so a strategy quoting the lower bound would send an approver to
a version the tool's own prerequisites reject. The dedicated documentation also now names the feature
**GitHub Copilot upgrade**, with *app modernization* surviving as the Visual Studio component name — worth
knowing when searching, and the reason both names appear above. The number is re-verified at approval on
§7.1 consequence 1's rule: a tooling minimum is current on a date, not permanently.
...
| — | 5 | `src/MvcMusicStore/wwwroot/lib/**`, `src/MvcMusicStore/wwwroot/js/shopping-cart.js`, `deploy/sql/SecurityAuditLog.sql`, `deploy/sql/SessionCache.sql`, `src/MvcMusicStore/Models/CartAlias.cs` | Artifacts a **consuming** deliverable has decided since the plan was frozen and given no path: the committed output of the acquisition mechanism §9.3 fixes; the externalized cart script [06 §10.2](06-azure-hosting-recommendations.md) requires under `script-src 'self'` and [05 §7.4](05-aspnet-core-migration-approach.md)'s token header travels on, whose source is a Razor view rather than an asset file — a derivation no glob over the asset directories would express; the durable audit table's DDL, which [06 §9.5](06-azure-hosting-recommendations.md) states in full while placing it nowhere; the **session cache table's DDL**, which becomes a repository file only because §6.3 withdraws `dotnet-sql-cache` over its packaged advisory and [06 §6.4](06-azure-hosting-recommendations.md) states the replacement text while placing it nowhere — the one addition of the five this checkpoint itself caused; and the cart-handoff alias **entity type**, which [05 §4.13](05-aspnet-core-migration-approach.md) declares as a mapped member of `MusicStoreEntities` and states in full — key, columns, widths, state column and index — while placing it nowhere, exactly as 06 does with the audit DDL. It is the one of the five that is a **CLR type** rather than a data or script file, which is why leaving it unlisted did not merely omit a path: this map forbids a public type with no containing file, so its absence contradicted the closure claim §12.2's count paragraph makes. Each row states the placement rule it follows, so a later operator script has a predictable home |
...
| **Total** | **34** | — | The additions §12.2's count adds to its 36 authoritative-map rows |
...
| `.config/dotnet-tools.json` | CREATE | **net-new** | `dotnet-ef` `8.0.30` — **one entry, because §6.3 withdraws `dotnet-sql-cache`** and the session cache table is created from reviewed T-SQL instead ([06 §6.4](06-azure-hosting-recommendations.md)) |
...
| `deploy/sql/SessionCache.sql` | CREATE | **net-new** — the same `git ls-files '*.sql'` evidence as the row below; the session cache table has no script in any edition because no edition has a session cache table | **The session cache table's reviewed DDL, and the second standalone deployment script this map carries.** It exists because §6.3 **withdraws** `dotnet-sql-cache`: the tool that used to create `dbo.SessionCache` ships a critical vulnerable dependency, so the object is created by reviewed idempotent T-SQL instead, exactly as the row below creates `dbo.SecurityAuditLog`. The DDL is the tool's own `script dbo SessionCache --idempotent` output, captured once at authoring time and **verified byte-identical between the tool's `8.0.30` and `10.0.11` releases**, so adopting the text costs nothing against what the tool would have executed. It is **not** an EF Core migration and adds none: [06 §6.4](06-azure-hosting-recommendations.md) applies it as stage **`M1`** of the provisioning order, under the deployment identity and inside that section's DDL window, so neither context's migration set and neither history table acquires it. Structurally identical to the row below — a `.sql` file under the repository-root `deploy/sql/` folder, compiled into nothing, packaged with no project, reaching the release runner as a repository file, adding no project, no item group and no edge to §12.4's graph. Its content, its `IF NOT EXISTS` guard and the step that applies it are [06 §6.4](06-azure-hosting-recommendations.md)'s; this row owns its existence, its path and its build treatment |
| `deploy/sql/SecurityAuditLog.sql` | CREATE | **net-new** — the repository has three `.sql` files and none of them is a deployment script: `git ls-files '*.sql'` returns only the edition schema scripts `src/MVC3/MvcMusicStore-Assets/Data/MvcMusicStore-Create.sql`, `src/MVC4/MvcMusicStore-Create.sql` and `src/MVC4/MvcMusicStore/MvcMusicStore-Create.sql` | **A standalone deployment script, and one of the rows §12.1's count of five consuming-deliverable artifacts refers to.** [06 §9.5](06-azure-hosting-recommendations.md) requires a durable audit table and states its DDL in full; what nothing stated until this row is *where that DDL lives*, which is the only reason it is mapped here. It is **not** an EF Core migration and adds none: [06 §6.3](06-azure-hosting-recommendations.md) applies it as its own step of the provisioning order, under the deployment identity and inside that section's DDL window, so neither context's migration set and neither history table acquires it and §12.5's placement of both migration sets is unchanged. Structurally: a `.sql` file under a repository-root `deploy/sql/` folder, **compiled into nothing and packaged with no project** — it is neither a `Compile` nor a `Content` item of any of the four projects of §5.6, so it adds no project, no item group and no edge to §12.4's graph, and it reaches the release runner as a repository file. Its content, its `OBJECT_ID` guard, its append-only grants and the step that applies it are [06 §9.5](06-azure-hosting-recommendations.md)'s and [06 §6.3](06-azure-hosting-recommendations.md)'s; this row owns its existence, its path and its build treatment |

**The row count, checkable, and every target appearing exactly once.** The table above is **one table of 70
rows**, with **70 distinct target cells** — no path, glob or artifact is named by two rows — of which **69
...
- **`1 + 4 + 13 + 15 = 33` additions, each with its sanction in §12.1's short record.** The **lockfile**
  ...
  four files in one row, one per project. **Five** rows are artifacts a *consuming* deliverable has since
  ...
  full and whose *location* nothing stated until this row; `deploy/sql/SessionCache.sql`, the session cache
  table's reviewed DDL, which becomes a repository file because §6.3 withdraws the tool that used to create
  that object and [06 §6.4](06-azure-hosting-recommendations.md) states the replacement DDL while placing it
  nowhere; and `Models/CartAlias.cs`, the cart-handoff
  alias entity type [05 §4.13](05-aspnet-core-migration-approach.md) declares as a mapped member of
  `MusicStoreEntities` and states in full while placing it nowhere — the only one of the five that is a
...
- **`36 + 34 = 70`.** Every row is one target and every target is one row, and **the map carries no
...
library, this edge moves to it, the operator tool's present count of **one** `PackageReference` item is
what changes with it — that one being `Microsoft.Extensions.Logging.ApplicationInsights` `2.23.0`, which
§7.2's own row declares for this project and this project only because the audit record needs a
destination, while the other fifteen of that table's sixteen rows belong to the web application or to a
test project — and nothing else in this section does.
...
**Half two — the two instruments the data-migration procedure uses, neither of which this target ships.**
...
| Reviewed T-SQL, executed by the deployment principal — **including the session cache table's DDL**, which reaches this row rather than a tool row of its own because §6.3 withdraws `dotnet-sql-cache` | SQL Server's, and the review is [03](03-modernization-roadmap.md)'s gate. **The executor that runs the statements is a release-runner prerequisite rather than a pin or a mapped file** — §12.7 states that boundary and [06 §6.10](06-azure-hosting-recommendations.md) owns the implementation, its version and its grammar | The same |

**Two rows where an earlier revision had three, and the row that went is the reason the remaining one grew.**
`dotnet sql-cache create` was the second instrument; with the tool withdrawn (§6.3), the object it created
is created by the third instrument instead, so the count of instruments falls by one while the work each
performs is unchanged. [06 §6.4](06-azure-hosting-recommendations.md) owns the DDL and
[06 §6.10.1](06-azure-hosting-recommendations.md) the client that executes it.
...
**So the complete answer to "what can be invoked" is four entries: `provision-admin`, `seed-catalog`,
`dotnet ef`, and reviewed T-SQL.** The six dispatching spellings of the two shipped
...
| A **second container image** — a migration or tooling image beside the conditional site image | §6.9.1 of [06](06-azure-hosting-recommendations.md), and the note below | There **is** a second image — 06 §6.9.1 selects a platform-started container job and §12.7.2 promotes a signed migration-executor image for it — and the exclusion is nevertheless correct, because that image is produced from a `Dockerfile` the release **writes into its own build context**, never a file in this tree. Two related readings are wrong and are named so this row is not cited for either: the migration route is not "a runner inside the virtual network" — 06 §12.3's `B9` withdraws exactly that phrasing, since the **execution surface** is inside the network and the **runner never is** — and it is not "not an image" either. **The map therefore contains exactly one image-producing *file*, the conditional `Dockerfile`**, which is a statement about this repository rather than about how many images the target runs |
| A **second operator project** — a separate console project for the data phases or for seeding | §12.6, §12.9 | Neither needs one. **Seeding is a guarded command of the web project** (§12.6), which already composes the application; **the data phases are an operational procedure** executed with `dotnet ef` and reviewed T-SQL (§12.9), which ships no executable at all. An operator capability is not a map row |
...
`Dockerfile`**, and an *unspecified* second image would be an artifact with no build, publish, test,
checksum or promotion chain anywhere in §13.4 — the half-specified invention the closure rule exists to
refuse.

**06 has since taken the third of those three mechanisms, and this row's outcome is unchanged for a reason
worth stating rather than leaving as a coincidence.** [06](06-azure-hosting-recommendations.md) §6.9.1
selects the platform-started job, and §12.7.2 specifies a **migration-executor image** for it — so the
target does run a second image, and the condition this note set has been met in full: 06 states that
image's base (a digest-pinned .NET 8 SDK image imported into the estate's own registry), its contents (the
promoted bundles and operator tool, the reviewed DDL scripts, the pinned tool manifest's one entry and the
orchestrator), its tag and digest handling (built by `az acr build`, digest read back and asserted, carried
in a provenance record) and its verification point (a severity-gated scan plus `notation verify` against a
strict trust policy). **The map still gains no row**, because that image is built from a `Dockerfile` the
release **writes into its own build context** rather than from a file in this tree — which is the
distinction the exclusion row now states explicitly. What *would* require an amendment here is 06 electing
to **commit** that Dockerfile, and nothing in 06 does.
...
pipeline or an operator session, using the migration bundle `dotnet ef` generates and
reviewed T-SQL against the source and target databases — two instruments, no bespoke executable, therefore no
...
One of the two instruments is a **pinned tool-manifest entry** — `dotnet-ef`
`8.0.30` in `.config/dotnet-tools.json` (§6.3), restored by `dotnet tool restore` — so a reader finds
it at a mapped path. The other is not like it: reviewed T-SQL is a **normative migration mechanism**
...
the four. §7.2's sixteen rows are therefore unchanged by it, and so is the tool manifest: **nothing is added
...
70-row map cannot otherwise tell an approved file from an invented one, and inferring the rule from the
...
- **Configuration reaches a host through the environment, never the command line; the two release-time
  ...
  - **A bundle and the T-SQL client take a connection argument, because neither has another input.** The
    ...
    since a settings file left beside the executable would then decide which database receives DDL. The
    T-SQL client that executes reviewed statements — including the session cache table's DDL, since §6.3
    withdraws the tool that used to create it — takes its target on the command line as
    [06 §6.10.1](06-azure-hosting-recommendations.md) specifies, and accepts configuration by no other
    route at all. So the prohibition binds
    ...
    **never echoed** into a log, a command trace or a failure message, and the server and database it
    ...
    the bundle invocation, and its §6.4 owns the cache-table DDL and the step that executes it.
...
dotnet tool restore                       # §6.3 — the manifest's one pinned tool, dotnet-ef
...
fails the build late, inside `az acr build`, with an error about a tag rather than about an input.
...
- **Nothing in the sequence tolerates a non-zero exit, and the absence of any tolerance is deliberate.**
  ...
  two `||` constructs that do appear are **guards that exit**, not suppressors. The build has no tolerated
  non-zero exit, and neither does the release step that creates the session cache table: with §6.3's tool
  withdrawn, that object is created by **idempotent** reviewed T-SQL whose `IF NOT EXISTS` guard succeeds
  whether or not the table is already present, so there is no *already present* status for a caller to
  tolerate. Its exit-code contract is [06](06-azure-hosting-recommendations.md) §6.4's, and
  [06 §6.10.1](06-azure-hosting-recommendations.md)'s for the client that runs it.
...
**The set is closed at exactly the files the Publication row above counts, and exactly one file a consumer
names is deliberately outside it, for a reason that is a lifecycle rather than an omission.** It is
**separately attached evidence**: it is not promoted, not counted in that set, not written
into `artifacts/` and gets no `.sha256` sidecar.
...
**One file where an earlier revision listed two, and the one that left the list changed category rather
than disappearing.** `sessioncache.sql` was the second entry: a file the cache-table tool emitted so that
an approver could read what the tool was about to execute, attached to a change record and executed by
nothing. With §6.3's withdrawal of `dotnet-sql-cache` there is no tool to emit it and no execution to
preview, and the reviewed DDL is a **committed repository file** instead —
`deploy/sql/SessionCache.sql`, mapped in §12.2 and applied as [06 §6.4](06-azure-hosting-recommendations.md)'s
stage `M1`. It is therefore neither a build artifact nor separately attached evidence: it is source, it is
reviewed once when it is committed rather than once per release, and the release **does** execute it —
which is the property the withdrawn entry explicitly denied. Nothing about the promoted set changes,
because that entry was never counted in it.
...
`deploy/sql/SessionCache.sql` is the reviewable DDL of the session cache table, and it is **not a build
artifact** — but it is not separately attached evidence either, and the distinction matters because an
earlier revision of this section placed it in that second category. It is **source**: a committed
repository file (§12.2), whose text is the `--idempotent` output of `dotnet sql-cache script dbo
SessionCache` captured once at authoring time and reviewed once, when it is committed, rather than
regenerated and re-reviewed per release. §6.3 records why the tool that emitted it is not itself pinned,
restored or shipped. [06 §6.4](06-azure-hosting-recommendations.md) owns the DDL text and applies the file
as its stage `M1`, under the deployment identity and inside that section's DDL window. Four consequences,
...
- **It has no producer in the sequence above, by design.** No step of §13.4 generates it, so it never
  appears in `artifacts/`, and adding it there would make the build produce a file no consumer verifies. A
  file the build does not produce is not a file the build promotes.
- **It gets no sidecar and no publication.** Integrity of a committed source file is the repository's — the
  commit it arrived in and the review that admitted it — so a seventh `.sha256` and a
  fourteenth published file would both be claims this document cannot keep.
- **Its retention is the repository's.** The 90-day-and-ten-runs window below governs *promoted*
  artifacts; a committed file lives as long as the branch does, and the two lifetimes are not the same one.
- **Exactly one path reaches the database object.** The release executes this file and nothing else creates
  `dbo.SessionCache` — no tool, no migration and no second script — which is the property 06 §6.4 requires,
  because two paths to one database object is how the two diverge.

So the closure is precise in both directions: **the promoted set is what the Publication row counts and no
file beyond it is promoted**, and the one evidence file is outside the set on purpose, with a named
...
# BUILT AND PUSHED IN AZURE, not by a container engine on this agent. `az acr build` is the
# mechanism 06 §12.3's B3 selects for exactly this image, and 06 §12.7.2.1 issues the same call for
# the migration-executor image: the agent hands over a context and a Dockerfile and never runs a
# daemon, so no `docker` binary is on the release path and none has to be version-pinned. The
# registry and the repository are named from the declared inputs above rather than repeated as
# literals, so one input is spelled one way in both commands.
az acr build --registry "$REGISTRY_NAME" \
    --image "$IMAGE_REPOSITORY:$RUN_ID" \
    --file Dockerfile \
    --platform linux \
    artifacts/site
DIGEST="$(az acr repository show -n "$REGISTRY_NAME" \
    --image "$IMAGE_REPOSITORY:$RUN_ID" --query digest -o tsv)"
...
Six properties make that a contract rather than a handful of commands, and each is sourced:
...
5. Every Microsoft-shipped .NET 8 package is pinned to **`8.0.30`**, re-verified at approval and moved as
   a set. `dotnet-ef` is a **separate tool pin** the runtime packages do not
   provide, and it is the tool manifest's **only** entry: §6.3 **withdraws** `dotnet-sql-cache` because its
   packaged graph ships a critical vulnerable dependency, and the session cache table is created from
   committed reviewed T-SQL instead (§6.3, §7, §12.2). The **six** test-tooling pins version independently of that band — with one
...
7. **Four** browser libraries are retained and two are removed with no successor; the four are **vendored
   ...
   deployment step fetches them (§9.2, §9.3, §9.4).
