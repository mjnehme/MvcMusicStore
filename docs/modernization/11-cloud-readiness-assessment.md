...
| **Caching — whether the per-request layout aggregate is cached, where it is held, its lifetime and its invalidation** | [06](06-azure-hosting-recommendations.md) §6.4 | Records the readiness consequence of the uncached aggregate: which tier it loads first (section 3.8.2). It names **no cache and no placement**, and states no sizing or coherence requirement, because the decision is 06's and 06 takes it. |
...
| **Where the data-protection key ring lives**, its protect-at-rest configuration, rotation policy and slot/revision isolation | [06](06-azure-hosting-recommendations.md) | States what each hosting shape supplies by default, the three properties an external store adds, and whether the requirement is advisable or mandatory (section 3.2). The location and the policy are **not decided here**. |
| **Where the data-protection key ring lives**, what each candidate platform's default ring does, its protect-at-rest behaviour, rotation policy and slot/revision isolation | [06](06-azure-hosting-recommendations.md) | States the requirement for persistent shared key storage and the consequence of not having it (section 3.2), and cites 06 for the platform defaults. The location is **not decided here**. |
...
**What the absent `<machineKey>` actually means, stated precisely rather than as "keys are regenerated every time".** With no element declared, ASP.NET's default is auto-generation with per-application isolation, and the generated material is **persisted** — held per machine, under the application-pool identity's own registry hive, and derived per application — so it is *not* freshly minted on every process start, and a claim that it is would not survive a reviewer. Three things do vary, and each is a readiness fact:

- **It varies by machine.** The material belongs to the host, so two hosts running the same deployment do not hold the same keys unless something outside the repository arranges it.
- **It varies by identity and by the persisted store's availability.** A change of application-pool identity, or a host configuration in which that identity's profile store is unavailable, produces new material rather than the previous material.
- **It is replaced whenever the host is.** On a platform that adds, recycles and replaces instances without notice, that is a routine event rather than an exceptional one.
...
The target framework supplies a data-protection stack without any package reference, which makes it easy to record as solved. It is not. The *stack* is provided; the *key ring* still has to be told **where to live** — and what happens when nobody tells it is not one behaviour but four. They are separated here because collapsing them into a single "per-instance and ephemeral" statement is wrong in both directions: it overstates the risk on one platform and understates it on the other.

| What is being described | What its key ring does | Owner of the fact |
| --- | --- | --- |
| **The three editions as they stand today** — this document's subject | No key configuration of any kind, so the framework's `<machineKey>` default applies: key material is **auto-generated once, isolated per application, and held by the operating system outside the worker process**, so it **survives an application-pool recycle** and is shared with nothing beyond that machine — not with a second instance, and not with a second machine | this document, below |
| **A generic or self-hosted ASP.NET Core host** | Filesystem-based and local to the machine, so it is unshared between hosts and lost with the directory | this document, as the framework baseline |
| **App Service** | Network-backed under the home content share, so the ring is shared across **all instances of a single deployment slot** and survives a restart. Separate slots do **not** share a ring, so a slot swap breaks decryption continuity | [06](06-azure-hosting-recommendations.md) §7.1 |
| **Container Apps** | Genuinely per-replica and ephemeral | [06](06-azure-hosting-recommendations.md) §7.1 |

Only the first two rows are claims this document makes. The two platform rows are hosting behaviours deliverable 06 owns per section 1.5, cited rather than restated — including the reasoning that makes an explicit store necessary even where the default already shares a ring.
...
1. **The key is generated once, not per process.** The framework generates random key material and the *operating system* holds it, in the machine's Local Security Authority store, outside the worker process and protected at rest by the platform.
2. **It is therefore stable across an application-pool recycle.** A recycle, a worker crash and an ordinary restart all leave the key in place, so a cookie issued before one is still decryptable after it. **This document previously said the opposite, and the correction matters**: the failure mode is not process lifetime.
...
**So the readiness conclusion is unchanged, and property 3 rather than property 2 is what carries it.** Key material is machine-scoped, so **nothing one instance issues is readable by another**, and **the readiness conclusion does not depend on which target is chosen**: the target needs an explicitly persisted, shared key ring on either platform — because the App Service default's sharing stops at a boundary the release process crosses, and because the Container Apps default shares nothing at all. A managed platform scales by adding *instances*, which is exactly the boundary a machine-local key does not cross; that it would survive a recycle on any one of them buys nothing.

#### 3.2.2 On the target platform the default is better than that — and still not sufficient

The target framework supplies a data-protection stack with no package reference, which makes it easy to record as solved. The stack is provided and its hosting defaults are genuinely capable; what they are not is uniform across hosting shapes, and the requirement below depends on which shape is selected.
...
**The item closes on the two deployment verifications 05 §2.4 specifies, and not before — two assertions, each with its own cause.** Behind the platform front end the application observes HTTPS and serves **without a redirect loop**, *because* forwarded-header processing is configured and the header arrives over a trusted path, and the cookies in that response carry **`Secure`**, *because* 05's three cookie policies (§6.1, §6.3, §7.1) are `Always` and mark the attribute unconditionally; and a spoofed forwarded header presented on an untrusted path is **not honoured**. Neither verification stands in for the other: forwarding could be configured while a cookie policy was left implicit, and the policies could be set while the scheme was never recovered.
...
**This document does not choose a remedy, and deliberately names no cache.** Whether the aggregate is cached, where, with what lifetime and how it is invalidated is a hosting and data-platform decision owned by deliverable [06](06-azure-hosting-recommendations.md) §6.4, which decides it explicitly; the performance debt itself is owned by deliverable [08](08-technical-debt-register.md). The readiness fact recorded here — and the only one this document owns — is that the database, not the compute tier, is where this workload lands first, so capacity planning for the managed database must account for per-request layout traffic rather than for order and checkout volume alone.
...
| 7 | HTTPS enforced at the edge, HSTS, explicit security headers, forwarded-scheme handling for redirection and absolute-URL generation, and `Secure` cookies by explicit policy — **required, and not established by being recommended**: it closes on 05 §2.4's wiring and the two deployment verifications it specifies, each with its own cause — no redirect loop because forwarding is configured, `Secure` cookies because 05's three policies (§6.1, §6.3, §7.1) say `Always` — within the trusted topology 06 §10 defines (section 3.6.3) | 3.6 | 05 (composition, cookie options), 06 (enforcement, trusted topology), 09 (cookie policy) |
...
# The first three commands below read the working tree, so their outputs are those of the
# committed checkout; they are non-empty while uncommitted edits are in flight.
...
| No shared key material today; the target platform's key-ring default is slot-scoped and unprotected at rest | 3.2 | 12, 06 (key-ring location and policy) |
| Local file-based database storage; three engines; MVC 3's absent `.sdf` and host-inherited credential store; 14 binaries totalling 43,376,640 bytes, 10 ignored-yet-tracked and 4 never excluded | 3.3 | 12, 05 (EF Core, data migration), 06 (provisioning), 08 (tracked binaries), 09 (credential stores) |
...
| Plain HTTP; no HSTS; no security header of any kind; forwarded-scheme handling required and unverified; MVC 3 configured against the Visual Studio Development Server | 3.6 | 12, 05 (composition), 06 (edge enforcement, trusted topology), 09 (cookie policy), 10 (the MVC 3 local host) |
| Path-casing mismatch; the case-dependent `.gitignore` corroboration | 3.7 | 06 (the hosting option it gates), 08 (the `NuGet.exe` debt entry) |
...
| No declared key material; the key ring's location and its host-dependent defaults | 3.2 | 12, 06 (key-ring location and policy) |
...
| Plain HTTP; no HSTS; no security header of any kind | 3.6 | 12, 06 (edge enforcement), 09 (cookie policy) |
| Path-casing mismatch; the `.gitignore` corroboration | 3.7 | 06 (the hosting option it gates), 08 (the `NuGet.exe` debt entry) |
| Uncached per-request aggregate; the database scales before the compute tier | 3.8.2 | 08 (performance debt), 06 §6.4 (the caching decision — placement, lifetime and invalidation) |
| No shared key material in any edition; keys auto-generated per machine and per application, and shareable with nothing beyond that machine | 3.2 | 12, 06 (key-ring location and policy, and what each candidate platform's default ring does) |
| File-attached catalog databases; three engines; MVC 3's inherited SQL credential store; 43,376,640 committed bytes | 3.3 | 12, 05 (EF Core, data migration), 06 (provisioning), 08 (tracked binaries), 09 (credential stores), 10 (per-edition topology and the open MVC 3 verification item) |
| Plain HTTP; no HSTS; no security header of any kind | 3.6 | 12, 06 (edge enforcement), 09 (cookie policy) |
| Path-casing mismatch; the `.gitignore` corroboration | 3.7 | 06 (the hosting option it gates), 08 §10.7 / F-08-28 (the ignore-rule mechanics and the `NuGet.exe` debt entry) |

---

*Deliverable 11 of 13. Consumes deliverables [01](01-architecture-overview.md) and [02](02-dependency-inventory.md); feeds deliverable [12](12-migration-blockers.md) alongside 09 and 10. Index and requirement map: [README](README.md). Every count in this document is reproducible by the command stated beside it, and no tracked repository file outside `docs/modernization/` was created, modified or deleted in its production — the gitignored restore and build output that the assessment did produce was removed, and its absence verified by the four commands of section 1.3.*
