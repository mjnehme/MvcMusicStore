...
identifier for any pin this repository ships, and no other document in this assessment asserts one for
those pins either** — deliverable [02 §8.2](02-dependency-inventory.md) states why (no scanner is part
of this environment and the repository holds no dependency-scanning configuration to cite) and gives
the route by which a reader obtains today's set from NuGet's own restore-time audit, with the bounds on
what such a result establishes; [02 §8.1](02-dependency-inventory.md) carries the aged-pin class with
the exact pins, and [02 §8.3](02-dependency-inventory.md) carries the scan the implementation phase
owes. **That statement is scoped to the current-state pins deliberately, because two deliverables do
assert advisory identifiers — each against a target-state subject this repository does not ship — and
between them they are this corpus's only owners of them.**
[04 §7.4](04-dotnet8-migration-strategy.md) carries the set against a **target-state transitive
package pin** reached only on the conditional incremental path — a version the plan selects rather
than one the repository already ships, verified at an exact version against primary sources, and named
so that its remediation floor can be held as a precondition on that path, which is the point of naming
it at all. [06 §12.3](06-azure-hosting-recommendations.md), with
[06 §6.10.1](06-azure-hosting-recommendations.md), carries the identifiers a **binary** scan measured
in the two privileged release-path utilities that estate would install, and names them so that each
utility's admission can be gated on the finding being closed rather than disclosed. **The two subjects
do not overlap and neither is a pin this repository ships**: one is a package a restore would resolve,
the other an executable an agent image would carry. Their identifiers, severities and floors are cited
from those sections and restated nowhere here, so a reader who searches this corpus for an advisory
identifier and lands in either is reading a target-state case rather than a contradiction of this one.
Where this document draws a security conclusion from a
dependency, it draws it from the pin and its position in the code — repository facts — and never from
an identifier or a severity nobody here measured. It does not rank the editions against each other as
products — MVC 4 and MVC 3 are retained read-only as historical references and as the behavioural
baseline, and neither is a migration target.
...
identifier, a severity or a count**, because [02 §8.2](02-dependency-inventory.md) asserts none for the
pins this repository ships and this document may not invent what its source declines to state (§1.2);
the two target-state advisory sets [04 §7.4](04-dotnet8-migration-strategy.md) and
[06 §12.3](06-azure-hosting-recommendations.md) own are a different subject (§1.2) and no finding here
rests on either. What each rests on instead is a pin and where that pin sits in
the code, which is a repository fact and is cited as one.
...
- **Nothing in the repository records, retains or gates on dependency-audit output** ([02
  ...
  that output, an audit of the pins this repository ships, exists anywhere in this assessment (§1.2)** —
  and reviewable is not the same as gated: a route a reader must choose to run does not fail a build,
  and nobody is obliged to run it. Deliverable 03 owns the gate that would change that, and
  [02 §8.3](02-dependency-inventory.md) states what the implementation phase must add for the gate to
...
- **Its three client-side pins are the oldest in the repository** — `jQuery` `1.5.1`
  ...
  is asserted for them here or in the source, per §1.2.
