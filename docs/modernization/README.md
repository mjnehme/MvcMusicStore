...
- `git diff --name-status ea2552d6eda7c20e9477a512e5c615665618cf35..HEAD` — exactly **thirteen `A` rows**, every one of them a file under `docs/modernization/`: no `M` row, no `D` row, no path outside this directory. The left side is the immutable pre-assessment revision, the last commit before this assessment began; the right side is named `HEAD` rather than a hash **because a document cannot cite the commit that creates it**, so no hash is invented for it and the reader resolves `HEAD` on the delivered checkout.
